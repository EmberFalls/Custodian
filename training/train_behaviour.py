...
def probability_metrics(labels, probabilities):
    ...
    report = classification_metrics(
        ...
        np.asarray(CLASSES)[predictions],
        CLASSES,
    )
    report["accuracy"] = float(accuracy_score(labels, predictions))
    for name in ("precision", "recall", "f1-score"):
        report["macro_" + name.replace("-score", "")] = report["classification_report"][
            "macro avg"
        ][name]
    report["multiclass_brier_score"] = float(
        np.mean(
            np.sum(
                (probabilities - np.eye(len(CLASSES))[labels]) ** 2,
                axis=1,
            )
        )
    )
    ...
    for lower in np.arange(0, 1, 0.1):
        mask = (confidence >= lower) & (
            confidence < lower + 0.1 if lower < 0.9 else confidence <= 1
        )
        if mask.any():
...
def write_json(path, content):
    Path(path).write_text(
        json.dumps(content, indent=2, sort_keys=True, allow_nan=False), encoding="utf-8"
    )
    ...
    *,
    ...
    if destination.exists() and any(destination.iterdir()):
        raise FileExistsError(
            f"refusing to overwrite existing artifacts: {destination}; use a new output directory"
        )
    ...
    output_table = (
        Path(processed_path)
        if processed_path
        else Path("data/processed") / (destination.name + ".parquet")
    )
    ...
    write_json(
        report_dir / (destination.name + ".json"),
        {
            **provenance,
            "splits": split_counts,
    ...
    if prepare_only:
        print(
            f"Prepared data only; no model or metrics fabricated. Table: {output_table}", flush=True
        )
        return output_table
    try:
        ...
        raise RuntimeError(
            f"XGBoost could not load. Prepared data was saved, but no model was trained. {exc}"
        ) from exc
    ...
    encoded = {
        role: part["label"].map({name: i for i, name in enumerate(CLASSES)}).to_numpy()
        for role, part in roles.items()
    }
    parameters = dict(
    ...
    manifest = {
        "artifact_format": "custodian.xgboost_package.v1",
        ...
        "library_versions": {
            name: version(name) for name in ("xgboost", "numpy", "scikit-learn", "pandas", "joblib")
        },
        "python_version": platform.python_version(),
        "dataset": provenance,
        "split_strategy": {
            "method": provenance["grouping"],
            "seed": split_seed,
            "roles": split_counts,
            "group_overlap": False,
        ...
        "balance_policy": {
            "method": "Inverse class-frequency training sample weights only; no oversampling/downsampling.",
            "training_weight_per_class": {
                name: float(weights[encoded["train"] == i][0]) for i, name in enumerate(CLASSES)
            },
        },
    ...
    write_json(
        destination / "class_mapping.json", {"classes": CLASSES, "source_to_class": CLASS_MAPPING}
    )
        ...
        json.dumps(
            {
                ...
                "training_seconds": training_seconds,
                "final_test": metrics["final_test_calibrated"],
            },
            indent=2,
        ),
        flush=True,
...
def main():
    ...
    parser.add_argument("--acknowledge-isolated-vm", action="store_true")
