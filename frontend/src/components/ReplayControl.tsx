...
export function ReplayControl({ status, captures, onComplete }: { status: RuntimeStatus | null; captures: CaptureCandidate[]; onComplete: () => Promise<void> }) {
  ...
  const captureReady = selectedCapture?.status === "ready"
  ...
  const createExport = async (format: "json" | "csv") => {
    ...
    try {
      const result = await api<{ filename: string }>("/api/v1/exports", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ format, anonymize: anonymizeExport }),
      });
  ...
  return <section className="panel replay-control">
    ...
    <label className="capture-input">Approved capture<select value={capture} onChange={(event) => setCapture(event.target.value)} disabled={busy || Boolean(status?.replay_running)}><option value="">Select a capture</option>{captures.map((item) => <option value={item.display_name} key={item.display_name}>{item.display_name} · {(item.status ?? "queued").toUpperCase()}</option>)}</select></label>
    ...
    {status?.progress != null ? <div className="replay-progress"><progress value={status.progress} max={1} aria-label="Capture processing progress" /><span>{status.rebuilding ? `Rebuilding ${((status.rebuild_progress ?? 0) * 100).toFixed(1)}%` : `${(status.progress * 100).toFixed(1)}% of capture file processed`} · {status.mode.toUpperCase()}{status.mode === "paced" ? ` · ${status.speed_multiplier}×` : ""}</span></div> : null}
    <div className="control-row"><button className="button button--primary" title={captureReady ? "Start passive replay" : "Validate this capture before starting"} onClick={() => void control("/api/v1/replay/start", { capture: capture.trim(), mode, speed_multiplier: speed })} disabled={busy || !capture.trim() || !captureReady || !status || Boolean(status?.replay_running)}>Start replay</button>
      <button className="button" onClick={() => void control("/api/v1/captures/validate", { capture: capture.trim() })} disabled={busy || !capture.trim() || Boolean(status?.replay_running)}>Validate</button>
      ...
      <button className="button button--quiet" onClick={() => void seekBy(-0.1)} disabled={busy || !status?.capture}>← 10%</button>
      <button className="button button--quiet" onClick={() => void seekBy(0.1)} disabled={busy || !status?.capture}>10% →</button>
    ...
    <div className="control-row export-row"><label className="export-privacy"><input type="checkbox" checked={anonymizeExport} onChange={(event) => setAnonymizeExport(event.target.checked)} />Anonymize endpoint addresses</label><button className="button button--quiet" onClick={() => void createExport("json")} disabled={busy}>Export JSON</button><button className="button button--quiet" onClick={() => void createExport("csv")} disabled={busy}>Export CSV</button></div>
