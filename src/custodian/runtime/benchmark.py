...
def benchmark(capture: Path, *, config_dir: Path, paced_speed: float = 1):
    ...
    for mode in ReplayMode:
        ...
        results.append(
            {
                "mode": mode.value,
                "paced_speed_multiplier": paced_speed if mode is ReplayMode.PACED else None,
        ...
        print(
            f"{mode.value}: {duration:.6f}s, {metrics['packets']} packets, {metrics['flows']} flows, "
            f"{metrics['inference_vectors']} inference vectors",
            flush=True,
        )
    return {
        ...
        "sha256": digest,
        "file_bytes": capture.stat().st_size,
        "machine": {
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=root / "reports/benchmarks/one-model-fastpcap.json"
    )
