...
from custodian.core.schemas import Endpoint, FlowRecord, NumericStats, PacketObservation
...
class MutableFlow:
    ...
    def snapshot(self, close_reason: FlowCloseReason | None = None) -> FlowRecord:
        initiator_direction = "unknown"
        if self.initiator == self.key.endpoint_a:
            initiator_direction = "a"
        elif self.initiator == self.key.endpoint_b:
            initiator_direction = "b"
        return FlowRecord(
            ...
            ip_version=self.key.endpoint_a.ip.version,
            initiator_direction=initiator_direction,
