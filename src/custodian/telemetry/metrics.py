...
class MetricsCollector:
    def __init__(self, sample_limit: int = 2000) -> None:
        ...
        self.malformed_frames = self.unsupported_frames = self.truncated_frames = 0
            ...
            self._rate_history.append(
                {
                    ...
                    "packets_per_second": self._rates[0],
                    "flows_per_second": self._rates[1],
    ...
    def record_parse_failure(self, status: str) -> None:
        if status == "malformed":
    ...
    def snapshot(self, *, interval_seconds: float = 0.25, force: bool = False) -> dict:
        ...
        with self._lock:
            ...
            rates = self._rates
            duration = (
                max((self._ended or now) - self._started, 0) if self._started is not None else 0
            )
            active_duration = max(
                duration
                - self._pause_seconds
                - (now - self._paused_at if self._paused_at is not None else 0),
                0,
            )
            ...
            self._cached = {
                ...
                "inference_vectors": self.inference_vectors,
                ...
                "processing_rates": {
                    "packets_per_second": rates[0] if active else 0,
                ...
                "average_processing_rates": {
                    ...
                    "mbps": counts[2] * 8 / 1_000_000 / active_duration
                    if active_duration
                    else None,
                },
                "capture_duration_seconds": observed_duration,
                "observed_average_mbps": counts[2] * 8 / 1_000_000 / observed_duration
