...
class ModelDetector:
    ...
    def detect_batch(self, vectors: list[FeatureVector]) -> list[DetectorVerdict]:
        ...
        return [
            self._verdict(vector, result)
            for vector, result in zip(vectors, predictions, strict=True)
        ]

    def _verdict(self, vector: FeatureVector, prediction) -> DetectorVerdict:
        class_name, raw_score, calibrated, scores, inference_ms = prediction
        try:
        ...
        return DetectorVerdict(
            ...
            available_evidence=tuple(
                name for name, available in vector.availability.items() if available
            ),
            evidence=evidence,
            required_evidence=(),
            missing_evidence=(),
            ...
            scores=scores,
            stage_timings_ms={"inference": inference_ms},
        )
