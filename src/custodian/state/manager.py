...
from custodian.flow.manager import FlowUpdate
from custodian.state.windows import TemporalSnapshot, TrafficEvent


class TemporalStateManager:
    def __init__(
        self, windows_seconds: tuple[int, ...] = (10, 60, 300), max_events: int = 200_000
    ) -> None:
    ...
    def observe(self, packet: PacketObservation, update: FlowUpdate) -> None:
        ...
        event = TrafficEvent(
            ...
            is_new_flow=update.is_new_flow,
            domains=self._domains(packet),
        )
    ...
    def snapshot(
        self, source_ip: str, destination_ip: str, observed_at: datetime, window_seconds: int
    ) -> TemporalSnapshot:
        ...
        connection_times = tuple(
            event.timestamp for event in new_flows if event.dst_ip == destination_ip
        )
        ...
        short = (
            (
                sum((end - start).total_seconds() <= 2 for start, end in flow_times.values())
                / len(flow_times)
            )
            if flow_times
            else None
        ...
        if len(bins) >= 2:
            ...
            burstiness = (
                sqrt(sum((value - mean) ** 2 for value in bins.values()) / len(bins)) / mean
            )
        ...
        return TemporalSnapshot(
            ...
            byte_count=sum(e.packet_length for e in outgoing),
            flow_count=len(new_flows),
            unique_destinations=len({e.dst_ip for e in outgoing}),
            ...
            source_counts_for_destination=sources_for_target,
            connection_timestamps=connection_times,
            recent_domains=tuple(domain for e in outgoing for domain in e.domains),
            target_packet_count=len(target_events),
            total_window_packets=total,
            history_seconds=(observed_at - outgoing[0].timestamp).total_seconds()
            if outgoing
            else 0,
            history_complete=self._dropped_until is None or self._dropped_until < cutoff,
            short_flow_ratio=short,
            udp_share=sum(e.protocol.value == "UDP" for e in target_events)
            / max(len(target_events), 1),
