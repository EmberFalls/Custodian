...
from custodian.features.behaviour_flow import flow_model_values
from custodian.features.schema import BEHAVIOUR_SCHEMA_VERSION, entropy_from_counts
...
class BehaviourFeatureExtractor:
    ...
    def extract(
        ...
        values = {
            ...
            "source_entropy_for_destination": entropy_from_counts(
                state.source_counts_for_destination
            ),
        ...
        payload_known = bool(
            flow.payload_size_stats
            and flow.payload_size_stats.count == outgoing_packets + incoming_packets
        )
        payload_out = (
            flow.payload_bytes_a_to_b if endpoint_a_is_source else flow.payload_bytes_b_to_a
        )
        payload_in = (
            flow.payload_bytes_b_to_a if endpoint_a_is_source else flow.payload_bytes_a_to_b
        )
        shared = flow_model_values(
            duration=duration,
            packets_out=outgoing_packets,
            packets_in=incoming_packets,
