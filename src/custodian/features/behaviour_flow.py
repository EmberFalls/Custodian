...
def pooled_payload_variance(n_out, n_in, bytes_out, bytes_in, std_out, std_in):
    ...
    out_sum = np.where(
        n_out > 0,
        np.maximum(n_out - 1, 0) * np.asarray(std_out) ** 2 + n_out * (mean_out - mean) ** 2,
        0,
    )
    in_sum = np.where(
        n_in > 0,
        np.maximum(n_in - 1, 0) * np.asarray(std_in) ** 2 + n_in * (mean_in - mean) ** 2,
        0,
    )
    return safe_ratio(out_sum + in_sum, n_out + n_in)


def flow_model_values(
    *, duration, packets_out, packets_in, payload_out, payload_in, payload_variance, iat_variance
):
    duration = np.asarray(duration, dtype=float)
    packets_out, packets_in = (
        np.asarray(packets_out, dtype=float),
        np.asarray(packets_in, dtype=float),
    )
    payload_out, payload_in = (
        np.asarray(payload_out, dtype=float),
        np.asarray(payload_in, dtype=float),
    )
    total_packets, total_payload = packets_out + packets_in, payload_out + payload_in
    return {
        "flow_duration_seconds": duration,
        "packets_outbound": packets_out,
        "packets_inbound": packets_in,
        "packet_count": total_packets,
        "payload_bytes_outbound": payload_out,
        "payload_bytes_inbound": payload_in,
