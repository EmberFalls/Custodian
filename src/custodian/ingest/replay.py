...
from custodian.ingest.pcap import CaptureReader, PcapFrame


class ReplayController:
    def __init__(
        self,
        adapter: CaptureReader,
        *,
        mode: ReplayMode = ReplayMode.PACED,
        speed_multiplier: float = 1.0,
        monotonic: Callable[[], float] = time.monotonic,
        sleeper: Callable[[float], None] | None = None,
        rebuild_until_fraction: float = 0.0,
        on_rebuild_complete: Callable[[], None] | None = None,
    ) -> None:
        if not math.isfinite(speed_multiplier) or speed_multiplier <= 0:
            raise ValueError("speed_multiplier must be finite and positive")
        if not math.isfinite(rebuild_until_fraction) or not 0 <= rebuild_until_fraction <= 1:
            raise ValueError("rebuild_until_fraction must be between 0 and 1")
        self.adapter = adapter
        self.mode = ReplayMode(mode)
        self.speed_multiplier = speed_multiplier
        self._monotonic = monotonic
        self._sleeper = sleeper
        self._rebuild_target_bytes = int(adapter.size_bytes * rebuild_until_fraction)
        self._rebuilding = self._rebuild_target_bytes > 0
    ...
    @property
    def rebuild_progress(self) -> float | None:
        if not self.rebuilding:
            return None
        return min(self.processed_bytes / max(self._rebuild_target_bytes, 1), 1.0)
    ...
    def frames(self, on_idle: Callable[[], None] | None = None) -> Iterator[PcapFrame]:
        ...
        for frame in self.adapter.frames():
            ...
            if (
                not rebuilding
                and self.mode is ReplayMode.PACED
                and previous_capture_time is not None
            ):
            ...
            if rebuilding and self.processed_bytes >= self._rebuild_target_bytes:
                with self._condition:
                    self._rebuilding = False
                if self._on_rebuild_complete:
                    self._on_rebuild_complete()
