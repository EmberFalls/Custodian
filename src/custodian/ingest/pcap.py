...
SUPPORTED_DATALINKS = frozenset({dpkt.pcap.DLT_EN10MB, dpkt.pcap.DLT_RAW, dpkt.pcap.DLT_LINUX_SLL})
        ...
        with self.path.open("rb") as stream:
            try:
                ...
                if datalink not in SUPPORTED_DATALINKS:
                    raise ValueError(
                        "unsupported link-layer type; Ethernet, Linux cooked, or raw IP is required"
                    )
